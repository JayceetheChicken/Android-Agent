...
export function buildLoopSystemPrompt(): string {
  return [
    ...
    'optionally stop_loading if loading=true, then use fetch_current_page_text as the fallback.',
    'If fetch_current_page_text also fails, explain honestly that the page is open but neither',
    'WebView DOM reading nor HTTP HTML fetching returned readable content.',
